namespace Tests.Unit
{
    ...
    {
        [Test]
        public void Can_Serialize_Master_Data_Only()
        {
            var instance = new ChildlessTestUdo();
            instance.AlphaNumericalField = @"SomeValue";

            var serializer = new GenericUdoSerializer<ChildlessTestUdo>();

            string xml = serializer.Serialize(instance);

            Assert.AreEqual(TestUdoConstants.ChildlessTestUdoXml, xml);
        }
    ...
    internal static class TestUdoConstants
    {
        ...
        public const string ChildlessTestUdoXml =
            @"<My_Name><U_AlphaNumericalField>SomeValue</U_AlphaNumericalField></My_Name>";

    }

    [UserTable(@"MyTableName", @"My Table Description", BoUTBTableType.bott_MasterData)]
    [UserObject(@"MyId", @"My Name", BoUDOObjType.boud_MasterData)]
    internal class ChildlessTestUdo
    {
        [UserField(BoFieldTypes.db_Alpha, BoFldSubTypes.st_None)]
        public string AlphaNumericalField { get; set; }
    }
